...
%
% Везде используется свой стиль "tao": жирный заголовок, ПРЯМОЙ текст тела
...
\newtheoremstyle{tao}{3pt}{3pt}{}{}{\bfseries}{.}{\newline}{}
\theoremstyle{tao}
...
% вовсе без номера) — для тех редких случаев, когда у Тао само замечание пронумеровано
% (например, Remark 3.3.19). Название оставлено отдельным от \remark, чтобы не менять
% смысл уже использующихся в разделах \begin{remark}...\end{remark}.
...
% остаются на одной строке с именем (пробовали ещё \penalty сразу после
% идентификатора, чтобы переносить предпочтительно там, но под \raggedright
% это заставляло переносить вообще все сигнатуры, даже короткие — отказались).
...
% по границе страницы (первая строка сигнатуры — внизу одной страницы,
% продолжение после \leanbreak — наверху следующей), что рвёт чтение одной
% сигнатуры пополам. \par\nobreak в начале уже запрещает разрыв ПЕРЕД этим
% \leansource (между ним и предыдущим абзацем/\leansource) — см.
% \nopagebreak в определениях axiom/theorem/... выше, тот же приём.
%
...
% \leanbreak на самом деле начинает НОВЫЙ абзац, и разрыв между ним и
% предыдущим — обычный межабзацный разрыв, который эти регистры не видят
% (проверено на минимальном примере — penalty-регистры эффекта не дают).
% \parbox делает всю сигнатуру одним неделимым боксом: TeX либо целиком
% помещает её на текущую страницу, либо целиком переносит на следующую.
...
%
% \hangindent/\hangafter выше применяется только к переносам, которые сама
% строка-абзац выбирает по ширине (автоматический перенос) — к
% принудительному разрыву \\ он не применяется (переносимая часть иначе
% начиналась бы вровень с "Lean:", без отступа). \hspace*{2em} здесь
% восполняет это вручную тем же отступом в 2em, что и hangindent выше —
% если когда-нибудь понадобится изменить отступ, менять оба значения разом.
...
% заливки и без изменения цвета. Ставится после формулировки аксиомы/теоремы/
...
% не мешать \leansource оставаться последней строкой внутри них (см.
% STYLE.md, "Ссылка на Lean").
%
...
% framed.sty именно для этого и существует (та же причина, по которой
% \leansource выше использует \parbox только для сигнатур, которые заведомо
% короче страницы, а не для сколь угодно длинного текста).
