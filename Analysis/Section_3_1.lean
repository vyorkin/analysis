...
- Пустое множество {syntax term}`∅`, синглтоны {syntax term}`{y}` и пары {syntax term}`{y,z}`
...
class SetTheory where
  ...
  -- 1) Если `x` входит в выделенное подмножество, то `x` входит и в исходное множество `A`.
  -- 2) Вхождение `x` в выделенное подмножество равносильно истинности предиката P для этого x.

  -- Итак, у нас есть аксиома выделения (specify, Axiom 3.6):
  ...
  -- Ключевое ограничение: результат всегда лежит внутри уже существующего множества `A`.
  -- Это делает `specify` безопасной (не даёт парадоксов вроде парадокса Рассела), но и слабой —
  -- вы не можете с её помощью получить множество, чьи элементы «не помещаются» в `A`.
  ...
  --
  -- Вот именно эту "дыру" и закрывает аксиома замены (replacement).
  -- Она ПОСТУЛИРУЕТ:
  ...
  -- Несмотря на это, коллекция всех таких образов `{y : ∃ x ∈ A, P(x,y)}` всё равно
  -- образует корректное множество `Set`.
  --
  ...
  -- specify — «сузить» существующее множество до подмножества (безопасная, слабая операция).
  -- replace — «спроецировать» элементы множества через функциональную связь наружу,
  --           в произвольные объекты, и получить новое множество образов (гораздо более сильная операция).

  -- Именно из-за этой силы аксиома замены (replacement_axiom) исторически и
  -- была добавлена Френкелем к системе Цермело: без неё нельзя было доказать существование
  -- даже довольно простых множеств.
  -- Например: {{0}, {1}, {2}, …} — множество всех синглтонов натуральных чисел.
  -- Axiom 3.4 (singleton) даёт {n} только по одному, для одного
  -- конкретного n за раз, а specify (Axiom 3.6) не годится, так как {n} — это не элемент ℕ и
  -- взять его подмножеством просто неоткуда.
  ...
  -- условию `hP` (однозначность/функциональность) и тому, что она стартует от
  -- уже существующего множества `A`, а не от «свойства вообще»
  -- (как в наивной, парадоксальной схеме понимания).

  -- Постулирует существование некоторого множества с именем `nat`.
  -- Само по себе это ничего не гарантирует — Object/Set этого недостаточно, чтобы
  -- сказать, что это «именно натуральные числа».
  nat : Set -- Axiom 3.8
  -- А вот это и делает nat «натуральными числами».
  nat_equiv : ℕ ≃ Subtype (mem . nat) -- Axiom 3.8
  -- Здесь:
  -- 1) `Subtype (mem . nat)` – подтип Object, состоящий из элементов, принадлежащих множеству `nat`.
  -- 2) `≃` — это `Equiv` из Mathlib: не просто утверждение «существует биекция», а:
  ...
  -- обратная функция `nat_equiv.invFun`, которая берёт элемент `nat` и возвращает, каким
  -- числом ℕ он является — и они гарантированно взаимно обратны.
...
--   right_inv := by decide
  ...
  -- Axiom 3.9 (регулярность/фундирование, foundation/regularity axiom)
  -- Запрещает бесконечные нисходящие цепочки принадлежности
  -- (… ∈ x₂ ∈ x₁ ∈ A) и, в частности, множество не может содержать само себя.
  --
  ...
  --
  -- Условие `hA : ∃ x, mem x A` — значит, что «A непусто».
  --
  -- У `A` есть элемент `x` и если этот `x` сам является
  -- множеством `S` (через приведение `set_to_object`), то
  -- `x` и `A` не имеют общих элементов — ни один элемент `A` не лежит внутри `S`.
  --
  ...
  -- Axiom 3.11
  -- В книге Tao называется "power set axiom", но по сути это
  -- аксиома существования множества функций между двумя множествами.
  --
  -- `pow X Y` — это множество, обозначаемое в книге `X^Y`, состоящее из ВСЕХ функций из `Y` в `X`
  -- (то есть `pow X Y` — это множественный аналог типа функций `Subtype (mem . Y) → Subtype (mem . X)`).
  --
  -- Но тут возникает проблема: `mem` требует, чтобы элемент Set имел тип Object, а функция
  -- `f : Y → X` имеет тип "функция" — совсем другой Lean-тип, поэтому `mem f (...)` даже
  -- не типизируется. Значит, прежде чем сказать «F ∈ pow X Y», функцию сначала нужно
  -- закодировать как Object (как ID в базе данных) — этим и займётся `function_to_object` ниже.
  pow : Set → Set → Set -- Axiom 3.11

  -- Инъективное вложение (`↪`, как и `set_to_object` для множеств) функций `Y → X` в `Object`.
  function_to_object (X : Set) (Y : Set) :
    (Subtype (mem . X) → Subtype (mem . Y)) ↪ Object -- Axiom 3.11

  -- `powerset_axiom` связывает эти два понятия:
  -- объект `F` лежит в `pow X Y` тогда и только тогда,
  -- когда `F` — это "код"/ID (через `function_to_object`) какой-то функции `f : Y → X`.
  --
  -- Почему это называется "power set": если взять в качестве `X` двухэлементное множество
  -- `{true, false}`, то функции `Y → {true, false}` — это ровно индикаторные (характеристические)
  -- функции подмножеств `Y`, то есть находятся во взаимно однозначном соответствии с подмножествами `Y`.
  -- Значит `pow {true,false} Y` "изоморфно" обычному степенному множеству `P(Y)`. Общая аксиома про
  -- множество функций — это обобщение обычной аксиомы степенного множества.
  --
  ...
  -- Axiom 3.12 — "большое" объединение (union of a set of sets), в отличие от `union_pair`
  -- (Axiom 3.5), который объединяет ровно два множества за раз.
  --
...
/-
  Финальная часть Exercise 3.1.12: сформулируйте и докажите разумный
  положительный результат, заменяющий приведённую выше теорему,
  который включает разности множеств.
-/
