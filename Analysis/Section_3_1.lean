...
class SetTheory where
  ...
  -- Тут возникает проблема: `mem` требует, чтобы элемент `Set` имел тип `Object`,
  -- но функция `f : Y → X` имеет тип "функция", поэтому `mem f (...)` даже не типизируется.
  -- Значит, прежде чем сказать `F ∈ pow X Y`, функцию сначала нужно
  -- закодировать как `Object` (как ID в базе данных) — это делает `function_to_object` ниже.
  ...
  -- то функции `Y → {true, false}` — это ровно индикаторные (характеристические)
