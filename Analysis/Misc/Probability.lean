...
-- Вероятность всего пространства (`⊤`) равна 1
@[simp]
...
-- Конечная аддитивность: вероятность объединения непересекающихся событий равна сумме их вероятностей
theorem disj_sup {E F : A} (hEF : Disjoint E F) : ℙ (E ⊔ F) = ℙ E + ℙ F := ℙ.prob_disj_sup hEF

-- Вероятность пустого события равна 0
@[simp]
theorem bot : ℙ ⊥ = 0 := by
...
-- Вероятность дополнения события: `ℙ Eᶜ = 1 - ℙ E`
@[simp]
...
-- Если `E ≤ F`, то вероятность `F` раскладывается на вероятность разности `F \ E` и вероятность `E`
...
-- Монотонность вероятностной меры: из `E ≤ F` следует `ℙ E ≤ ℙ F`
theorem mono {E F : A} (hEF : E ≤ F) : ℙ E ≤ ℙ F := by
  linarith [ℙ.nonneg (F \ E), ℙ.ge_eq_add_diff hEF]

-- Формула включений-исключений для двух событий: `ℙ(E ⊔ F) + ℙ(E ⊓ F) = ℙ E + ℙ F`
...
-- Субаддитивность вероятностной меры: `ℙ(E ⊔ F) ≤ ℙ E + ℙ F`
...
-- Разворачивает `f.preserves ℙ ℙ'` в равенство вероятностей: `ℙ' (f E) = ℙ E` для конкретного `E`
@[simp]
