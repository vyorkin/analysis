...
/-! Формализация доказательства проблемы Эрдёша \#379, возникшего из обсуждений между Stijn
Cambie, Vjeko Kovac и Terry Tao. См. обсуждение по адресу https://www.erdosproblems.com/forum/thread/379 -/
...
/-- Если $$`p^\{a+b+1\} \mid n$$, то $$p^\{a+1\} \mid \binom\{n\}\{k\}$$ или $$p^\{b+1\} \mid k`. -/
...
/-- Если $$`p^r \mid n-1$$ и $$k,n-k$$ не делятся на $$p$$, то $$p^r \mid \binom\{n\}\{k\}`. -/
...
/-- Если $$`n=2^\{\phi(p^R)\}$$ и $$p>2^\{r-1\}$$, то $$2^r \mid \binom\{n\}\{k\}$$ или $$p^R \mid \binom\{n\}\{k\}`. -/
...
/-- Условие {lean}`1<n` необходимо, чтобы диапазон {lit}`k` не оказался пустым. -/
...
/-- Если $$p>2^\{r-1\}$$, то $$S(2^\{\phi(p^R)\}) \ge r$$. -/
...
/-- Положительное решение проблемы Эрдёша \#379. -/
