...
/-- Определение 4.3.1 (Абсолютная величина) -/
abbrev abs (x : ℚ) : ℚ :=
  if x > 0
  then x
  else (if x < 0 then -x else 0)

/-- Определение 4.3.1 (Абсолютная величина) (случай положительного числа) -/
theorem abs_of_pos {x : ℚ} (hx : 0 < x) : abs x = x := by grind

/-- Определение 4.3.1 (Абсолютная величина) (случай отрицательного числа) -/
theorem abs_of_neg {x : ℚ} (hx : x < 0) : abs x = -x := by grind

/-- Определение 4.3.1 (Абсолютная величина) (случай нуля) -/
...
/--
  Определение 4.3.2 (Расстояние).
  Здесь мы избегаем понятия расстояния из Mathlib, потому что оно вещественнозначное.
-/
theorem dist_eq (x y : ℚ) : dist x y = |x-y| := rfl

/-- Утверждение 4.3.3(a) (неотрицательность) / Упражнение 4.3.1 -/
theorem abs_nonneg (x : ℚ) : |x| ≥ 0 := by sorry

/-- Утверждение 4.3.3(a) (обращается в ноль тогда и только тогда, когда само число ноль) / Упражнение 4.3.1 -/
theorem abs_eq_zero_iff (x : ℚ) : |x| = 0 ↔ x = 0 := by sorry

/-- Утверждение 4.3.3(b) / Упражнение 4.3.1 -/
theorem abs_add (x y : ℚ) : |x + y| ≤ |x| + |y| := by sorry

/-- Утверждение 4.3.3(c) (двусторонняя оценка) / Упражнение 4.3.1 -/
theorem abs_le_iff (x y : ℚ) : -y ≤ x ∧ x ≤ y ↔ |x| ≤ y := by sorry

/-- Утверждение 4.3.3(c) (ограничено собственной абсолютной величиной) / Упражнение 4.3.1 -/
theorem le_abs (x : ℚ) : -|x| ≤ x ∧ x ≤ |x| := by sorry

/-- Утверждение 4.3.3(d) (мультипликативность) / Упражнение 4.3.1 -/
theorem abs_mul (x y : ℚ) : |x * y| = |x| * |y| := by sorry

/-- Утверждение 4.3.3(d) (отрицание) / Упражнение 4.3.1 -/
theorem abs_neg (x : ℚ) : |-x| = |x| := by sorry

/-- Утверждение 4.3.3(e) (неотрицательность) / Упражнение 4.3.1 -/
theorem dist_nonneg (x y : ℚ) : dist x y ≥ 0 := by sorry

/-- Утверждение 4.3.3(e) (обращается в ноль тогда и только тогда, когда числа равны) / Упражнение 4.3.1 -/
theorem dist_eq_zero_iff (x y : ℚ) : dist x y = 0 ↔ x = y := by
  sorry

/-- Утверждение 4.3.3(f) / Упражнение 4.3.1 -/
theorem dist_symm (x y : ℚ) : dist x y = dist y x := by sorry

/-- Утверждение 4.3.3(g) / Упражнение 4.3.1 -/
theorem dist_le (x y z : ℚ) : dist x z ≤ dist x y + dist y z := by sorry

/--
  Определение 4.3.4 (ε-близость).
...
/-- Этого варианта Утверждения 4.3.7(h) не было в учебнике,
    но он может пригодиться в некоторых последующих упражнениях. -/
theorem close_mul_mul' {ε δ x y z w : ℚ} (hxy : ε.Close x y) (hzw : δ.Close z w) :
  (ε*|z|+δ*|y|).Close (x * z) (y * w) := by
    sorry

/-- Определение 4.3.9 (возведение в степень) (базовый случай).
    Здесь мы используем определение из Mathlib. -/
lemma pow_zero (x : ℚ) : x^0 = 1 := _root_.pow_zero x

example : (0 : ℚ)^0 = 1 := pow_zero 0

/-- Определение 4.3.9 (возведение в степень) (индуктивный шаг).
    Здесь мы используем определение из Mathlib. -/
lemma pow_succ (x : ℚ) (n : ℕ) : x^(n+1) = x^n * x :=
  _root_.pow_succ x n

/-- Утверждение 4.3.10(a) (Свойства возведения в степень, I) (сумма показателей) / Упражнение 4.3.3 -/
theorem pow_add (x : ℚ) (m n : ℕ) : x^n * x^m = x^(n+m) := by sorry

/-- Утверждение 4.3.10(a) (Свойства возведения в степень, I) (произведение показателей) / Упражнение 4.3.3 -/
theorem pow_mul (x : ℚ) (m n : ℕ) : (x^n)^m = x^(n*m) := by sorry

/-- Утверждение 4.3.10(a) (Свойства возведения в степень, I) (произведение оснований) / Упражнение 4.3.3 -/
theorem mul_pow (x y : ℚ) (n : ℕ) : (x*y)^n = x^n * y^n := by sorry

/-- Утверждение 4.3.10(b) (Свойства возведения в степень, I) / Упражнение 4.3.3 -/
theorem pow_eq_zero (x : ℚ) (n : ℕ) (hn : 0 < n) : x^n = 0 ↔ x = 0 := by sorry

/-- Утверждение 4.3.10(c) (Свойства возведения в степень, I) (неотрицательность) / Упражнение 4.3.3 -/
theorem pow_nonneg {x : ℚ} (n : ℕ) (hx : x ≥ 0) : x^n ≥ 0 := by sorry

/-- Утверждение 4.3.10(c) (Свойства возведения в степень, I) (положительность) / Упражнение 4.3.3 -/
theorem pow_pos {x : ℚ} (n : ℕ) (hx : x > 0) : x^n > 0 := by sorry

/-- Утверждение 4.3.10(c) (Свойства возведения в степень, I) (монотонность) / Упражнение 4.3.3 -/
theorem pow_ge_pow (x y : ℚ) (n : ℕ) (hxy : x ≥ y) (hy : y ≥ 0) : x^n ≥ y^n := by sorry

/-- Утверждение 4.3.10(c) (Свойства возведения в степень, I) (строгое неравенство) / Упражнение 4.3.3 -/
theorem pow_gt_pow (x y : ℚ) (n : ℕ) (hxy : x > y) (hy : y ≥ 0) (hn : n > 0) : x^n > y^n := by sorry

/-- Утверждение 4.3.10(d) (Свойства возведения в степень, I) / Упражнение 4.3.3 -/
theorem pow_abs (x : ℚ) (n : ℕ) : |x|^n = |x^n| := by sorry

/--
  Определение 4.3.11 (Возведение в отрицательную степень).
...
/-- Утверждение 4.3.12(a) (Свойства возведения в степень, II) (сумма показателей) / Упражнение 4.3.4 -/
theorem zpow_add (x : ℚ) (n m : ℤ) (hx : x ≠ 0) : x^n * x^m = x^(n+m) := by sorry

/-- Утверждение 4.3.12(a) (Свойства возведения в степень, II) (произведение показателей) / Упражнение 4.3.4 -/
theorem zpow_mul (x : ℚ) (n m : ℤ) : (x^n)^m = x^(n*m) := by sorry

/-- Утверждение 4.3.12(a) (Свойства возведения в степень, II) (произведение оснований) / Упражнение 4.3.4 -/
theorem mul_zpow (x y : ℚ) (n : ℤ) : (x*y)^n = x^n * y^n := by sorry

/-- Утверждение 4.3.12(b) (Свойства возведения в степень, II) (положительность) / Упражнение 4.3.4 -/
theorem zpow_pos {x : ℚ} (n : ℤ) (hx : x > 0) : x^n > 0 := by sorry

/-- Утверждение 4.3.12(b) (Свойства возведения в степень, II) (монотонность) / Упражнение 4.3.4 -/
...
/-- Утверждение 4.3.12(c) (Свойства возведения в степень, II) / Упражнение 4.3.4 -/
theorem zpow_inj {x y : ℚ} {n : ℤ} (hx : x > 0) (hy : y > 0) (hn : n ≠ 0) (hxy : x^n = y^n) : x = y := by
  sorry

/-- Утверждение 4.3.12(d) (Свойства возведения в степень, II) / Упражнение 4.3.4 -/
