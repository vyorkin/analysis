...
- Определение целых чисел "раздела 4.1", `Section_4_1.Int`,
  как формальных разностей `a —— b` натуральных чисел `a b:ℕ`,
  с точностью до эквивалентности.
  (Это фактор вспомогательного типа `Section_4_1.PreInt`,
  состоящего из формальных разностей без какой-либо наложенной эквивалентности.)
...
-- Целые числа нельзя получить, просто взяв ℕ "с вычитанием":
-- в ℕ вычитание не всегда имеет смысл
-- (скажем, `3 - 5` — не натуральное число).
--
-- Стандартный обходной путь такой:
-- сперва завести "формальные разности" — пары `(a, b)`,
-- обозначающие потенциальное значение `a - b`,
-- даже когда самого вычитания ещё не существует, —
-- а затем отдельно отождествить те пары,
-- которые должны означать одно и то же целое число
-- (например, `(3,5)` и `(10,12)` обе должны означать `-2`).
--
-- `PreInt` — это тип таких ещё не отождествлённых пар.
-- `minuend`/`subtrahend` — обычные арифметические термины:
-- в выражении `a - b` число `a` называется уменьшаемым (minuend),
-- а `b` — вычитаемым (subtrahend).
structure PreInt where
  minuend : ℕ    -- Уменьшаемое
  subtrahend : ℕ -- Вычитаемое

-- `Setoid α` — это тип `α`, снабжённый отношением эквивалентности:
-- 1) Отношением `r : α → α → Prop`
--    (после объявления инстанса записывается инфиксно как `a ≈ b`).
-- 2) Доказательством `iseqv : Equivalence r`,
--    то есть рефлексивности, симметричности и транзитивности `r`.
--
-- Сам по себе `Setoid` ничего не "склеивает" — это лишь данные, необходимые для
-- последующего построения фактор-типа через `Quotient` (см. ниже, после `PreInt.eq`).
--
-- Здесь мы объявляем, когда две формальные разности `(a,b)` и `(c,d)` должны считаться
-- одним и тем же целым числом: по смыслу это `a - b = c - d`.
-- Но раз вычитание в ℕ не определено, условие переписывается в равносильном виде, использующем только сложение:
-- `a - b = c - d  ↔  a + d = c + b`.
/-- Определение 4.1.1 -/
instance PreInt.instSetoid : Setoid PreInt where
  r a b := a.minuend + b.subtrahend = b.minuend + a.subtrahend
  iseqv := {
    -- Рефлексивность: (a,b) ≈ (a,b), то есть a + b = a + b.
    refl := by sorry
    -- Симметричность: если (a,b) ≈ (c,d), то (c,d) ≈ (a,b).
    symm := by sorry
    -- Транзитивность: если (a,b) ≈ (c,d) и (c,d) ≈ (e,f), то (a,b) ≈ (e,f).
    trans := by
      -- Это доказательство написано так, чтобы следовать структуре оригинального текста.
      -- `intro` сразу разбирает все три `PreInt` на поля через паттерн-матчинг;
      -- `simp_all` затем сводит проекции вида `{minuend:=a,...}.minuend` к самим `a,b,c,d,e,f`,
      -- оставляя чистые арифметические гипотезы `h1 : a + d = c + b`, `h2 : c + f = e + d`
      -- и цель `a + f = e + b`.
      intro ⟨ a,b ⟩ ⟨ c,d ⟩ ⟨ e,f ⟩ h1 h2; simp_all
      -- Складываем обе гипотезы почленно: `congrArg₂ (·+·) h1 h2` даёt
      -- `(a+d)+(c+f) = (c+b)+(e+d)` (левые части сложены с левыми, правые — с правыми);
      -- `simp` убирает возникающий при этом бета-редекс.
      have h3 := congrArg₂ (· + ·) h1 h2; simp at h3
      -- Перегруппировываем слагаемые по обе стороны так, чтобы выделить общее
      -- слагаемое `(c + d)`, которое затем можно будет сократить.
      have : (a + f) + (c + d) = (e + b) + (c + d) := calc
        (a + f) + (c + d) = a + d + (c + f) := by abel
        _ = c + b + (e + d) := h3
        _ = (e + b) + (c + d) := by abel
      -- Сокращаем одинаковое слагаемое `(c + d)` справа — получаем ровно то, что нужно.
      exact Nat.add_right_cancel this
    }

-- Эквивалентность формальных разностей `PreInt` в явном арифметическом виде: `(a,b) ≈ (c,d) ↔ a + d = c + b`.
-- Доказывается тактикой `rfl`: это буквально определение `r` из `PreInt.instSetoid` выше,
-- лишь записанное через уже подставленные поля `⟨a,b⟩`/`⟨c,d⟩` — вычислять ничего не нужно.
@[simp]
theorem PreInt.eq (a b c d : ℕ) : (⟨ a,b ⟩ : PreInt) ≈ ⟨ c,d ⟩ ↔ a + d = c + b := by rfl

-- `Quotient s` для `s : Setoid α` — это фактор-тип: тип, элементы которого суть классы
-- эквивалентности элементов `α` по отношению `s.r`. Два элемента `α`, эквивалентные по `s.r`,
-- в `Quotient s` становятся буквально ОДНИМ и тем же элементом — это гарантируется на уровне
-- ядра Lean (аксиома `Quotient.sound : a ≈ b → Quotient.mk s a = Quotient.mk s b`
-- и обратное к ней `Quotient.exact`), а не просто доказуемо равными post-factum.
--
-- Отсюда и определение целых чисел: `Int` — это в точности формальные разности `PreInt`,
-- у которых "склеены" все эквивалентные (по `PreInt.instSetoid`) пары.
abbrev Int := Quotient PreInt.instSetoid

-- `Quotient.mk s x` — это класс эквивалентности элемента `x` в фактор-типе `Quotient s`,
-- то есть операция, переводящая формальную разность `(a,b) : PreInt`
-- в соответствующее ей целое число.
abbrev Int.formalDiff (a b : ℕ)  : Int := Quotient.mk PreInt.instSetoid ⟨ a,b ⟩

-- Обозначение `a —— b` для `Int.formalDiff a b`, имитирующее `a - b` из книги Тао.
-- Обычный минус здесь использовать нельзя — он уже занят под настоящее вычитание в кольце
-- `Int`, которое будет определено позже как раз через эту формальную разность.
infix:100 " —— " => Int.formalDiff

/-- Определение 4.1.1 (Целые числа, равенство) -/
...
/-- Определение 4.1.1 (Целые числа, существование представления) -/
theorem Int.eq_diff (n : Int) : ∃ a b, n = a —— b := by apply n.ind _; intro ⟨ a, b ⟩; use a, b

/-- Лемма 4.1.3 (Сложение определено корректно) -/
...
/-- Определение 4.1.2 (Определение сложения) -/
theorem Int.add_eq (a b c d : ℕ) : a —— b + c —— d = (a+c)——(b+d) := Quotient.lift₂_mk _ _ _ _

/-- Лемма 4.1.3 (Умножение определено корректно, левый аргумент) -/
...
/-- Лемма 4.1.3 (Умножение определено корректно, правый аргумент) -/
...
/-- Лемма 4.1.3 (Умножение определено корректно, оба аргумента) -/
...
/-- Определение 4.1.2 (Умножение целых чисел) -/
...
/-- Определение 4.1.4 (Отрицание целых чисел) / Упражнение 4.1.2 -/
...
/-- Лемма 4.1.5 (трихотомия целых чисел). -/
...
/-- Утверждение 4.1.6 (законы алгебры, коммутативное кольцо) / Упражнение 4.1.4 -/
...
/-- Утверждение 4.1.8 (Отсутствие делителей нуля) / Упражнение 4.1.5 -/
theorem Int.mul_eq_zero {a b : Int} (h : a * b = 0) : a = 0 ∨ b = 0 := by sorry

/-- Следствие 4.1.9 (Закон сокращения) / Упражнение 4.1.6 -/
theorem Int.mul_right_cancel₀ (a b c : Int) (h : a*c = b*c) (hc : c ≠ 0) : a = b := by sorry

/-- Определение 4.1.10 (Порядок на целых числах, нестрогий) -/
instance Int.instLE : LE Int where
  le n m := ∃ a : ℕ, m = n + a

/-- Определение 4.1.10 (Порядок на целых числах, строгий) -/
instance Int.instLT : LT Int where
  lt n m := n ≤ m ∧ n ≠ m

-- Разворачивает `≤` в определение (Определение 4.1.10): `a ≤ b` означает, что `b` получается из `a` прибавлением некоторого натурального `t`
...
/-- Лемма 4.1.11(a) (Свойства порядка) / Упражнение 4.1.7 -/
theorem Int.lt_iff_exists_positive_difference (a b : Int) : a < b ↔ ∃ n : ℕ, n ≠ 0 ∧ b = a + n := by sorry

/-- Лемма 4.1.11(b) (Сложение сохраняет порядок) / Упражнение 4.1.7 -/
theorem Int.add_lt_add_right {a b : Int} (c : Int) (h : a < b) : a+c < b+c := by sorry

/-- Лемма 4.1.11(c) (Умножение на положительное сохраняет порядок) / Упражнение 4.1.7 -/
theorem Int.mul_lt_mul_of_pos_right {a b c : Int} (hab : a < b) (hc : 0 < c) : a*c < b*c := by sorry

/-- Лемма 4.1.11(d) (Отрицание обращает порядок, строгий) / Упражнение 4.1.7 -/
theorem Int.neg_gt_neg {a b : Int} (h : b < a) : -a < -b := by sorry

/-- Лемма 4.1.11(d) (Отрицание обращает порядок, нестрогий) / Упражнение 4.1.7 -/
theorem Int.neg_ge_neg {a b : Int} (h : b ≤ a) : -a ≤ -b := by sorry

/-- Лемма 4.1.11(e) (Порядок транзитивен) / Упражнение 4.1.7 -/
theorem Int.lt_trans {a b c : Int} (hab : a < b) (hbc : b < c) : a < c := by sorry

/-- Лемма 4.1.11(f) (Трихотомия порядка, утверждение) / Упражнение 4.1.7 -/
theorem Int.trichotomous' (a b : Int) : a > b ∨ a < b ∨ a = b := by sorry

/-- Лемма 4.1.11(f) (Трихотомия порядка, не больше и меньше одновременно) / Упражнение 4.1.7 -/
theorem Int.not_gt_and_lt (a b : Int) : ¬ (a > b ∧ a < b):= by sorry

/-- Лемма 4.1.11(f) (Трихотомия порядка, не больше и равно одновременно) / Упражнение 4.1.7 -/
theorem Int.not_gt_and_eq (a b : Int) : ¬ (a > b ∧ a = b):= by sorry

/-- Лемма 4.1.11(f) (Трихотомия порядка, не меньше и равно одновременно) / Упражнение 4.1.7 -/
...
/-- Упражнение 4.1.9. Квадрат любого целого числа неотрицателен. -/
